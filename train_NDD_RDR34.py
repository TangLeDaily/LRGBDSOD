...
# model name
from rootmodel.ResNet34_NDD_FFM import *
model = LRGBDSOD()
...
parser.add_argument("--This_name", default="ResNet34_NDD_FFM", type=str) # wandb run name & model save name path
...
def train(optimizer, model, criterion, epoch, train_loader):
    ...
    for iteration, batch in enumerate(train_loader):
        ...
        out_RGB, out = model(images, depths)
        loss = 0.2*criterion(out_RGB, gts)+0.8*criterion(out, gts)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
